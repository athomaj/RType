...
void ScorePage::drawBackGround(sf::RenderWindow &window, bool mouseClicked)
{
...
void ScorePage::addScore(const std::string nameScore)
{
  if (writeBox.size() < 10)
    {
      sf::Vector2f pos(textScore[writeBox.size()].getPosition().x + 0.03f * (float)width, textScore[writeBox.size()].getPosition().y - 0.001f * (float)height);
      writeBox[nameScore] = new WriteInBox(pos, 30, 11);
      writeBox[nameScore]->addString(nameScore);
    }
}
